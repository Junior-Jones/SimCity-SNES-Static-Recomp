...
int sc_v34_group_000E9(SCV11Runtime *r,uint32_t address,uint8_t m,uint8_t x,uint8_t e){
  ...
  case 0x03A7DCu:
    if(m==0u&&x==0u&&e==0u){ /* EE 19 0E Mesen corrected gameplay frame 4435; Version 34 gameplay route */
      sc_v11_scheduler_code_access(r,0x03A7DCu,3u);r->open_bus=0x0Eu;
  ...
  case 0x03A7DFu:
    if(m==0u&&x==0u&&e==0u){ /* 60 Mesen corrected gameplay frame 4435; Version 34 gameplay route */
      sc_v11_scheduler_code_access(r,0x03A7DFu,1u);r->open_bus=0x60u;
      r->scheduler.cpu_instructions++;r->generated_callbacks++;r->machine.static_executed_instructions=(uint32_t)r->scheduler.cpu_instructions;c->pc=0xA7E0u;
      {uint16_t q;sc_v11_scheduler_internal_cycle(r,2u);q=sc_v11_pop16(r);sc_v11_scheduler_internal_cycle(r,1u);c->pc=(uint16_t)(q+1u);}
      if(!sc_v11_finish_instruction(r))return 0;
      return r->route_failed?0:1;
    }
