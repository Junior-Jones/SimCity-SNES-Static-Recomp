...
int sc_v25_static_cpu_step(SCV11Runtime *r,uint32_t address,uint8_t m,uint8_t x,uint8_t e){
  ...
  case 0x00D2DFu:
    if(m==1u&&x==1u&&e==0u){ /* C2 20 REP #$20 */
      sc_v11_scheduler_code_access(r,address,2u);r->open_bus=0x20u;
  ...
  case 0x00D2E1u:
    if(m==0u&&x==1u&&e==0u){ /* A9 00 00 LDA #$0000 */
      sc_v11_scheduler_code_access(r,address,3u);r->open_bus=0x00u;
  ...
  case 0x00D2E4u:
    if(m==0u&&x==1u&&e==0u){ /* 02 00 COP #$00 */
      sc_v11_scheduler_code_access(r,address,2u);r->open_bus=0x00u;
