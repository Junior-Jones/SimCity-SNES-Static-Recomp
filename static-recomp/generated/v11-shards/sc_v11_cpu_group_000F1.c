...
int sc_v11_cpu_group_000F1(SCV11Runtime *r,uint32_t address,uint8_t m,uint8_t x){
  ...
  case 0x03C5EDu:
    if(m==0u&&x==0u){ /* AD4000 LDA abs; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C5EDu,3u);r->open_bus=0x00u;
  ...
  case 0x03C5F0u:
    if(m==0u&&x==0u){ /* 48 PHA imp; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C5F0u,1u);r->open_bus=0x48u;
  ...
  case 0x03C5F1u:
    if(m==0u&&x==0u){ /* A90800 LDA imm; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C5F1u,3u);r->open_bus=0x00u;
  ...
  case 0x03C5F4u:
    if(m==0u&&x==0u){ /* 8D4000 STA abs; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C5F4u,3u);r->open_bus=0x00u;
  ...
  case 0x03C5F7u:
    if(m==0u&&x==0u){ /* 202ECE JSR abs; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C5F7u,3u);r->open_bus=0xCEu;
  ...
  case 0x03C5FCu:
    if(m==0u&&x==0u){ /* 68 PLA imp; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C5FCu,1u);r->open_bus=0x68u;
  ...
  case 0x03C5FDu:
    if(m==0u&&x==0u){ /* 8D4000 STA abs; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C5FDu,3u);r->open_bus=0x00u;
  ...
  case 0x03C600u:
    if(m==0u&&x==0u){ /* A01000 LDY imm; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C600u,3u);r->open_bus=0x00u;
  ...
  case 0x03C603u:
    if(m==0u&&x==0u){ /* 2019CF JSR abs; Version 15 B-mode entry island */
      sc_v11_scheduler_code_access(r,0x03C603u,3u);r->open_bus=0xCFu;
  ...
  case 0x03C438u:
    if(m==0u&&x==0u){ /* 8A TXA; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C438u,1u);r->open_bus=0x8Au;
      r->scheduler.cpu_instructions++;r->generated_callbacks++;r->machine.static_executed_instructions=(uint32_t)r->scheduler.cpu_instructions;c->pc=0xC439u;
      c->a=c->x;sc_v11_set_nz16(c,c->a);sc_v11_scheduler_idle_or_read(r);
      if(!sc_v11_finish_instruction(r))return 0;
      return r->route_failed?0:1;
    }
  ...
  case 0x03C439u:
    if(m==0u&&x==0u){ /* 18 CLC; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C439u,1u);r->open_bus=0x18u;
      r->scheduler.cpu_instructions++;r->generated_callbacks++;r->machine.static_executed_instructions=(uint32_t)r->scheduler.cpu_instructions;c->pc=0xC43Au;
      c->p=(uint8_t)(c->p&((uint8_t)~SC_FLAG_C));sc_v11_scheduler_idle_or_read(r);
      if(!sc_v11_finish_instruction(r))return 0;
      return r->route_failed?0:1;
    }
  ...
  case 0x03C43Au:
    if(m==0u&&x==0u){ /* 690600 ADC imm; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C43Au,3u);r->open_bus=0x00u;
  ...
  case 0x03C43Du:
    if(m==0u&&x==0u){ /* AA TAX; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C43Du,1u);r->open_bus=0xAAu;
      r->scheduler.cpu_instructions++;r->generated_callbacks++;r->machine.static_executed_instructions=(uint32_t)r->scheduler.cpu_instructions;c->pc=0xC43Eu;
      c->x=c->a;sc_v11_set_nz16(c,c->x);sc_v11_scheduler_idle_or_read(r);
      if(!sc_v11_finish_instruction(r))return 0;
      return r->route_failed?0:1;
    }
  ...
  case 0x03C43Eu:
    if(m==0u&&x==0u){ /* E03C00 CPX imm; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C43Eu,3u);r->open_bus=0x00u;
      r->scheduler.cpu_instructions++;r->generated_callbacks++;r->machine.static_executed_instructions=(uint32_t)r->scheduler.cpu_instructions;c->pc=0xC441u;
      sc_v11_op_compare(r,c->x,0x003Cu,16u);
      if(!sc_v11_finish_instruction(r))return 0;
      return r->route_failed?0:1;
    }
  ...
  case 0x03C441u:
    if(m==0u&&x==0u){ /* D0ED branch; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C441u,2u);r->open_bus=0xEDu;
      r->scheduler.cpu_instructions++;r->generated_callbacks++;r->machine.static_executed_instructions=(uint32_t)r->scheduler.cpu_instructions;c->pc=0xC443u;
  ...
  case 0x03C443u:
    if(m==0u&&x==0u){ /* A20000 LDX imm; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C443u,3u);r->open_bus=0x00u;
      r->scheduler.cpu_instructions++;r->generated_callbacks++;r->machine.static_executed_instructions=(uint32_t)r->scheduler.cpu_instructions;c->pc=0xC446u;
      sc_v11_op_ldx(r,0u,16u);
      if(!sc_v11_finish_instruction(r))return 0;
      return r->route_failed?0:1;
    }
  ...
  case 0x03C446u:
    if(m==0u&&x==0u){ /* BDF30C LDA abs,x; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C446u,3u);r->open_bus=0x0Cu;
  ...
  case 0x03C449u:
    if(m==0u&&x==0u){ /* 9DED0C STA abs,x; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C449u,3u);r->open_bus=0x0Cu;
  ...
  case 0x03C44Cu:
    if(m==0u&&x==0u){ /* BDF50C LDA abs,x; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C44Cu,3u);r->open_bus=0x0Cu;
  ...
  case 0x03C44Fu:
    if(m==0u&&x==0u){ /* 9DEF0C STA abs,x; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C44Fu,3u);r->open_bus=0x0Cu;
  ...
  case 0x03C452u:
    if(m==0u&&x==0u){ /* BDF70C LDA abs,x; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C452u,3u);r->open_bus=0x0Cu;
  ...
  case 0x03C455u:
    if(m==0u&&x==0u){ /* 9DF10C STA abs,x; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C455u,3u);r->open_bus=0x0Cu;
  ...
  case 0x03C458u:
    if(m==0u&&x==0u){ /* 8A TXA; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C458u,1u);r->open_bus=0x8Au;
      r->scheduler.cpu_instructions++;r->generated_callbacks++;r->machine.static_executed_instructions=(uint32_t)r->scheduler.cpu_instructions;c->pc=0xC459u;
      c->a=c->x;sc_v11_set_nz16(c,c->a);sc_v11_scheduler_idle_or_read(r);
      if(!sc_v11_finish_instruction(r))return 0;
      return r->route_failed?0:1;
    }
  ...
  case 0x03C459u:
    if(m==0u&&x==0u){ /* 18 CLC; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C459u,1u);r->open_bus=0x18u;
  ...
  case 0x03C45Au:
    if(m==0u&&x==0u){ /* 690600 ADC imm; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C45Au,3u);r->open_bus=0x00u;
  ...
  case 0x03C45Du:
    if(m==0u&&x==0u){ /* AA TAX; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C45Du,1u);r->open_bus=0xAAu;
  ...
  case 0x03C45Eu:
    if(m==0u&&x==0u){ /* E03600 CPX imm; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C45Eu,3u);r->open_bus=0x00u;
  ...
  case 0x03C461u:
    if(m==0u&&x==0u){ /* D0E3 branch; Version 15 B-mode city slot island */
      sc_v11_scheduler_code_access(r,0x03C461u,2u);r->open_bus=0xE3u;
